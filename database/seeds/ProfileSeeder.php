...
class ProfileSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        factory(App\User::class,5)->create()->each(function ($user) use ($rates, $districts, $hairs, $appearances){

            factory(App\Profile::class, 20)->create([
                'user_id' => $user->id,
            ])->each(function ($profile) use ($rates, $districts, $hairs, $appearances){
                $profile->rates()->attach(
                    $rates->random(rand(1, 3))->pluck('id')->toArray()
                );

                $profile->hairs()->attach(
                    $rates->random(rand(1, 5))->pluck('id')->toArray()
                );

                $profile->appearances()->attach(
                    $rates->random(rand(1, 4))->pluck('id')->toArray()
                );
